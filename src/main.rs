...
#[inline(always)]
fn tnf() -> chrono::format::DelayedFormat<chrono::format::StrftimeItems<'static>> {
    ...
    while let Some(mut stream) = acceptor.accept().await {
        tokio::spawn({
            ...
            async move {
                ...
                println!("[tunnel {addr}] target connected [{}]: {peer_addr} -> {target}", tnf());
...
async fn main() -> Result<()> {
    ...
    println!("[system {pid}] {} starting [{}]", cli::VERSION, tnf());
    ...
    for rt in args.route_table {
        ...
        handles.push(tokio::spawn(async move {
            ...
            loop {
                attempt += 1;
                eprintln!("[server {rta}] attempt #{attempt} [{}]", tnf());

                match run_server(&rt.tunn, rt.pairs.clone()).await {
                    Ok(_) => {
                        println!("[server {rta}] exited normally [{}]", tnf());
                    }
                    Err(e) => {
                        eprintln!("[server {rta}] crashed [{}]: {e}", tnf());
                    }
                }
    ...
    for t in args.tunn {
        ...
        handles.push(tokio::spawn(async move {
            ...
            loop {
                attempt += 1;
                eprintln!("[tunnel {ta}] attempt #{attempt} [{}]", tnf());

                match open_tunn(&t).await {
                    Ok(_) => {
                        eprintln!("[tunnel {ta}] exited normally [{}]", tnf());
                    }
                    Err(e) => {
                        eprintln!("[tunnel {ta}] crashed [{}]: {e}", tnf());
                    }
                }
    ...
    println!("[system {pid}] startup complete [{}]", tnf());
    ...
    println!("[system {pid}] shutdown complete [{}]", tnf());
